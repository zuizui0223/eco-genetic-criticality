"""Command-line runner for conservation-preserving H1 fragment projection audit."""
...
from causal_model.finite_h1_fragment_projection_audit import (
    ...
    write_finite_h1_fragment_projection_artifacts,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Project finite H1 full states into fragmentation landscapes with declared invariants.")
    parser.add_argument("--profile", choices=tuple(_FACTORIES), default=PROFILE_STANDARD)
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/h1_fragment_projection"))
...
def run_from_namespace(args: argparse.Namespace) -> dict[str, Path]:
    ...
    prefix = args.prefix or f"h1_fragment_projection_{spec.profile}_seeds_{seed_tag}"
    ...
    write_finite_h1_fragment_projection_artifacts(cells, csv_path=csv_path, json_path=json_path)
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    manifest = {
        "runner": "causal_model.finite_h1_fragment_projection_audit_cli",
        "campaign": "finite_h1_fragment_projection_v1",
        ...
        "projection_rule": {
            "absolute_variables": ["total_population", "trait_bin_totals"],
            "intensive_variables": ["population_weighted_high_allele_frequency", "population_weighted_interaction"],
            "population_allocation": "positive deterministic largest-remainder allocation proportional to target patch area",
            "trait_allocation": "deterministic integer contingency allocation with target-patch population row margins and source trait-bin column margins",
            "area_requirement": "source and target total area must match",
            "migration_interpretation": "projection does not apply migration; equal_migrating differs only through its subsequent scenario dynamics",
        },
        "selection_policy": "Every declared seed, pair, and replicate is retained. Missing H1 full-state preparation remains unavailable rather than being filtered out.",
